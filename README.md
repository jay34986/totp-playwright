...
1. CloudFormation の出力から以下のURLを組み立ててWebブラウザでアクセスする。
  `<CognitoHostedUiDomain>/signup?client_id=<UserPoolClientId>&response_type=token&scope=openid+email+profile&redirect_uri=<CloudFrontUrl>/`
2. サインアップ後、Hosted UI の案内に従って TOTP を登録
3. MFA設定時に表示される Show secret key からシークレット（Base32）を確認し安全な場所に控える
...
2. 値を設定
   ...
   - `TEST_USERNAME` / `TEST_PASSWORD` : Hosted UIで登録したメールアドレスとパスワード
   - `TOTP_SECRET` : MFA登録時に取得した Base32 シークレット
...
実行前に Node.js バージョンを確認します。

```bash
node -v
```

`v20.5.x` など古いマイナーバージョンでは、Playwright の ESM ローダー初期化で失敗する場合があります。
可能であれば Node.js 22 LTS を使用してください。
...
5. E2E テスト（Playwright + otplib）
   ...
   ローカル Node.js の更新がすぐにできない場合は、次で互換ランタイムを一時利用できます。

   ```bash
   npx -y node@22.15.0 $(command -v npm) run test:e2e:headed
   ```
